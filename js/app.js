angular.module('defects4j-website', ['ngRoute', 'ui.bootstrap', 'anguFixedHeaderTable'])
	...
	]).directive('bicdiff', ['$http', function ($http) {
		return {
			restrict: 'A',
	...
	}]).directive('bfcdiff', ['$http', function ($http) {
		return {
			restrict: 'A',
			link: function (scope, elem, attrs) {
				scope.$watch("$ctrl.bug.bfcdiff", function () {
					var diff2htmlUi = new Diff2HtmlUI({ diff: scope["$ctrl"].bug.bfcdiff });
					diff2htmlUi.draw($(elem), {
						inputFormat: 'java',
						drawFileList: true,
						matching: 'lines'
	...
	.controller('bugModal', function($rootScope, $uibModalInstance, bug, classifications) {
		...
		$ctrl.actionName = function (actionId) {
			return getName('Repair Operators', actionId);
		};
		$ctrl.repairName = function (key) {
			var repairNames = $ctrl.classifications['Runtime Information']["Automated Program Repair"];
	...
	.controller('mainController', function($scope, $rootScope, $location, $window, $rootScope, $http, $uibModal) {
		...
		$scope.match  = "all";
		$scope.filter   = {};
		$scope.pageTitle = "RegMiner4APR Regression Bugs";
		...
		$http.get("data/regminer4apr-regressionbugs.json").then(function (response) {
